...
public class OrderDaoMockTest {
    ...
    @BeforeEach
    public void before() {
        orderDao = new JdbcOrderDaoImpl(jdbcTemplate, getIdRowMapper, orderLogRowMapper);
    ...
    public void testIsFoundMemberID() {
        //given
        var memberId = 1L;
        when(jdbcTemplate.queryForObject(JdbcOrderDaoImpl.GET_MEMBER_BY_ID, memberBooleanRowMapper, memberId)).thenReturn(null);
        //when
    ...
    public void testIsThisOrderLogExist() {
        ...
        when(jdbcTemplate.queryForObject(JdbcOrderDaoImpl.FIND_ORDER_LOG, booleanOrderLogRowMapper, foundId)).thenReturn(null);
        //when
