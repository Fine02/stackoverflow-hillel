...
public class DownVotedCommentServiceTest {
    ...
    public void whenCommentIsNotFoundThenThrowsCommentNotFoundException() {
        ...
        //when
        try{
            commentAfterVoting = voteCommentService.downVote(comment, member);
        }
        //then
        catch(Exception e){
            exception = e;
    ...
    public void whenMemberIsAlreadyVotedTheCommentThenThrowsAlreadyVotedException(){
        ...
        wantToVoteMember.getDownVotedComments().add(ID1);
        ...
        //when
        try{
            commentAfterVoting = voteCommentService.downVote(comment, wantToVoteMember);
        }
        //then
        catch(Exception e){
            exception = e;
    ...
    @Test
    public void whenMemberVotesTheCommentThenVoteCountDecrementAndAddReputation() throws DataBaseOperationException {
        var wantToVoteMember = mockMember(ID1);
        var author = mockMember(ID2);
        var comment = mockComment(ID1, author);
        when(commentData.findById(ID1)).thenReturn(Optional.of(comment));
        ...
        assertTrue(wantToVoteMember.getDownVotedComments().contains(ID1));
