/*
 * This file is generated by jOOQ.
 */
package com.ra.course.com.stackoverflow.entity.jooq.tables.records;


import com.ra.course.com.stackoverflow.entity.jooq.tables.CommentTable;
import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record7;
import org.jooq.Row7;
import org.jooq.impl.UpdatableRecordImpl;

import java.time.LocalDateTime;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class CommentRecord extends UpdatableRecordImpl<CommentRecord> implements Record7<Long, String, LocalDateTime, Integer, Long, Long, Long> {

    private static final long serialVersionUID = 367763709;

    /**
     * Setter for <code>public.comment.id</code>.
     */
    public void setId(Long value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.comment.id</code>.
     */
    public Long getId() {
        return (Long) get(0);
    }

    /**
     * Setter for <code>public.comment.comment_text</code>.
     */
    public void setCommentText(String value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.comment.comment_text</code>.
     */
    public String getCommentText() {
        return (String) get(1);
    }

    /**
     * Setter for <code>public.comment.creation_date</code>.
     */
    public void setCreationDate(LocalDateTime value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.comment.creation_date</code>.
     */
    public LocalDateTime getCreationDate() {
        return (LocalDateTime) get(2);
    }

    /**
     * Setter for <code>public.comment.vote_count</code>.
     */
    public void setVoteCount(Integer value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.comment.vote_count</code>.
     */
    public Integer getVoteCount() {
        return (Integer) get(3);
    }

    /**
     * Setter for <code>public.comment.author_id</code>.
     */
    public void setAuthorId(Long value) {
        set(4, value);
    }

    /**
     * Getter for <code>public.comment.author_id</code>.
     */
    public Long getAuthorId() {
        return (Long) get(4);
    }

    /**
     * Setter for <code>public.comment.answer_id</code>.
     */
    public void setAnswerId(Long value) {
        set(5, value);
    }

    /**
     * Getter for <code>public.comment.answer_id</code>.
     */
    public Long getAnswerId() {
        return (Long) get(5);
    }

    /**
     * Setter for <code>public.comment.question_id</code>.
     */
    public void setQuestionId(Long value) {
        set(6, value);
    }

    /**
     * Getter for <code>public.comment.question_id</code>.
     */
    public Long getQuestionId() {
        return (Long) get(6);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    public Record1<Long> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record7 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row7<Long, String, LocalDateTime, Integer, Long, Long, Long> fieldsRow() {
        return (Row7) super.fieldsRow();
    }

    @Override
    public Row7<Long, String, LocalDateTime, Integer, Long, Long, Long> valuesRow() {
        return (Row7) super.valuesRow();
    }

    @Override
    public Field<Long> field1() {
        return CommentTable.COMMENT_TABLE.ID;
    }

    @Override
    public Field<String> field2() {
        return CommentTable.COMMENT_TABLE.COMMENT_TEXT;
    }

    @Override
    public Field<LocalDateTime> field3() {
        return CommentTable.COMMENT_TABLE.CREATION_DATE;
    }

    @Override
    public Field<Integer> field4() {
        return CommentTable.COMMENT_TABLE.VOTE_COUNT;
    }

    @Override
    public Field<Long> field5() {
        return CommentTable.COMMENT_TABLE.AUTHOR_ID;
    }

    @Override
    public Field<Long> field6() {
        return CommentTable.COMMENT_TABLE.ANSWER_ID;
    }

    @Override
    public Field<Long> field7() {
        return CommentTable.COMMENT_TABLE.QUESTION_ID;
    }

    @Override
    public Long component1() {
        return getId();
    }

    @Override
    public String component2() {
        return getCommentText();
    }

    @Override
    public LocalDateTime component3() {
        return getCreationDate();
    }

    @Override
    public Integer component4() {
        return getVoteCount();
    }

    @Override
    public Long component5() {
        return getAuthorId();
    }

    @Override
    public Long component6() {
        return getAnswerId();
    }

    @Override
    public Long component7() {
        return getQuestionId();
    }

    @Override
    public Long value1() {
        return getId();
    }

    @Override
    public String value2() {
        return getCommentText();
    }

    @Override
    public LocalDateTime value3() {
        return getCreationDate();
    }

    @Override
    public Integer value4() {
        return getVoteCount();
    }

    @Override
    public Long value5() {
        return getAuthorId();
    }

    @Override
    public Long value6() {
        return getAnswerId();
    }

    @Override
    public Long value7() {
        return getQuestionId();
    }

    @Override
    public CommentRecord value1(Long value) {
        setId(value);
        return this;
    }

    @Override
    public CommentRecord value2(String value) {
        setCommentText(value);
        return this;
    }

    @Override
    public CommentRecord value3(LocalDateTime value) {
        setCreationDate(value);
        return this;
    }

    @Override
    public CommentRecord value4(Integer value) {
        setVoteCount(value);
        return this;
    }

    @Override
    public CommentRecord value5(Long value) {
        setAuthorId(value);
        return this;
    }

    @Override
    public CommentRecord value6(Long value) {
        setAnswerId(value);
        return this;
    }

    @Override
    public CommentRecord value7(Long value) {
        setQuestionId(value);
        return this;
    }

    @Override
    public CommentRecord values(Long value1, String value2, LocalDateTime value3, Integer value4, Long value5, Long value6, Long value7) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached CommentRecord
     */
    public CommentRecord() {
        super(CommentTable.COMMENT_TABLE);
    }

    /**
     * Create a detached, initialised CommentRecord
     */
    public CommentRecord(Long id, String commentText, LocalDateTime creationDate, Integer voteCount, Long authorId, Long answerId, Long questionId) {
        super(CommentTable.COMMENT_TABLE);

        set(0, id);
        set(1, commentText);
        set(2, creationDate);
        set(3, voteCount);
        set(4, authorId);
        set(5, answerId);
        set(6, questionId);
    }
}
